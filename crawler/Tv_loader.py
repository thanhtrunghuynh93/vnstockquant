...
class TvDatafeed:
    ...
    def get_hist(self, symbol: str, exchange: str = '', interval: Interval = Interval.in_daily, n_bars: int = 10, fut_contract: int = None) -> pd.DataFrame:
        """get historical data
...
def get_hist(ticker, exchange, interval = Interval.in_1_hour, hist_len=100):
    ...
    for i in range(numtry):
        try:
            df = crawler.get_hist(symbol=ticker,exchange=exchange,interval=interval, n_bars = hist_len)
            df["ticker"] = ticker
            return df
        except:
